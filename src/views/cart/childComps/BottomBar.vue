<template>
  <div class="bottom-menu">
    ...
    <span class="buy-product" @click="sum">去计算({{heji}})</span>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    heji() {
      let ccc = this.$store.getters.cartList.filter(item => item.checked === true )
        return ccc.length
      }
  },
  methods: {
    ...
    sum() {
      const ccc= this.$store.getters.cartList.filter(item => item.checked === true )
